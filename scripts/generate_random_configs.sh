...
#
# This script creates new YAML files in configs/ with suffix _rnd.yaml,
# updates:
...
usage() {
  cat <<'EOF'
Usage:
  scripts/generate_random_configs.sh [options]
...
Options:
  ...
  --suffix <text>          Suffix for generated configs/run_name (default: _rnd)
  --dry-run                Show actions without writing files or submitting jobs.
  -h, --help               Show this help.

Notes:
  - Resolver value is random.
...
for src in files:
    ...
    cfg.env.conflict_resolution = "random"
